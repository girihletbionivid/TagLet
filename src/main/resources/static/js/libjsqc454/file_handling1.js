...
{
	var idForParentDiv = $("#" + elementId).closest("div").prop("id");
...
function addSubCategory(subCategory, pathValue, element,flag) {
	if (flag) {
		...
		+ k
		+ "' ><div class='col-sm-1'></div><div class=' col-sm-11  classWhiteColor '> "
		+ "<a href='#' class='glyphicon glyphicon-file iconSmallClass colorTextFile' >" + subCategory
		+ "</a></div>";
	}else{
...
function setSelectedValue(element) {
	if (document.getElementById("valueSelectedDseq").value != "") {
		...
		//alert(typeOfReplacemnent+"ddddd==s=======>"+jsonArrayForIndexLocation[indexOfReplacement][0].right);
		if( numberOfDiv != undefined )
		{
			var tempArr = arrOfFileList[numberOfDiv].split(";");
			tempArr[idxOfElement] = jarray["" + document.getElementById("valueSelectedDseq").value];
			var tempVar = tempArr.join(";");
			arrOfFileList[numberOfDiv] = tempVar;
		}
