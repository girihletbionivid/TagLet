...
function submitFormTrinity() {
	// generateReportForm()
	// alert("get value for===>"+toString(jsonArrayForIndexLocation));
	// getRightFiles();
	trinityOutput = "";
	getLeftFiles();
	...
	if (idInputReadType == "Paired") {
		...
		if (validateProjectNameTrinity() && validateCpu()
			...
			if (refferenceAndHybridTrinity()) {
				...
				var rightFile = getRightFiles();
				var leftFile = getLeftFiles();
				formData.append("rightReadFile", getRightFiles());
				formData.append("leftReadFile", getLeftFiles());
				var maxIntron = document.getElementById("idMaxIntron").value;
				formData.append("maxIntron", maxIntron);
				var idMinExpressedCov = document.getElementById("idMinExpressedCov").value;
				formData.append("minExpressedCov", idMinExpressedCov);
		...
		if (validateCpu() && validateMinContigLength() && validateMinKmerCov()
				&& validateMaxMemory() && validateOptionalFiles()
				&& validateOptionalFiles() && validateOutputTrinity()) {
			// $("#myModal").modal('show');
			var formData = new FormData();
			var extention = "a";
			if (sequenceType == "Fasta")
			...
			if (refferenceAndHybridTrinity()) {
				...
				formData.append("singleReadFile", getSingleFiles());
...
$(document).ready(function() {
	...
	$("#selectOptionalFormGroup4").fadeOut();
	document.getElementById("idProjectName").innerHTML="";
	jsonArrayForIndexLocation = {};
	setHeightToThis(document.getElementById("trinityCmdOutput"));
	document.getElementById("idOptionalSelect1").checked = false;
	document.getElementById("idOptionalSelect2").checked = false;
...
function onloadFunctionLoadData() {
	...
	if (localStorage.pid != null) {
		...
		} else if (localStorage.tagName == "assembly_validation") {
			// alert("::xxx"+localStorage.tagName);
...
function getLeftFiles() {
	var leftFiles="";
	var seperator="";
	//alert("hey apan=======>"+toString(jsonArrayForIndexLocation));
	for (var i in jsonArrayForIndexLocation) {
		leftFiles = leftFiles +seperator + jsonArrayForIndexLocation[i][0].left;
		seperator=",";
		
	}
	if(document.getElementById("idLeftFileSelect").innerHTML!="No file selected")
	leftFiles = leftFiles+ seperator
			+ jarray[document.getElementById("idLeftFileSelect").innerHTML];
	seperator=",";
//	alert("===getLeftFiles()=>"+leftFiles);
	return leftFiles;
}
function getRightFiles() {
	var rightFiles="";
	var seperator="";
	for (var i in jsonArrayForIndexLocation) {
		rightFiles = rightFiles + seperator + jsonArrayForIndexLocation[i][0].right;
		seperator=",";
	}
	if(document.getElementById("idRightFileSelect").innerHTML!="No file selected")
	rightFiles = rightFiles + seperator
			+ jarray[document.getElementById("idRightFileSelect").innerHTML];
	seperator=",";
	return rightFiles;
}
function getSingleFiles() {
	var singleFiles="";
	var seperator="";
	for (var i in jsonArrayForIndexLocation) {
		singleFiles = singleFiles + seperator
				+ jsonArrayForIndexLocation[i][0].single;
		seperator=",";
	}
	if(document.getElementById("idSingleFileSelect").innerHTML!="No file selected")
	singleFiles = singleFiles + seperator
			+ jarray[document.getElementById("idSingleFileSelect").innerHTML];
	seperator=",";
	return singleFiles;
}
		...
		if (idInputReadType == "Paired") {
			document
					...
					.setAttribute("onclick",
							"getAllDirectory('idRightFileSelect','fasta,fasta_filtered,fa');");
			document
					.getElementById("idLeftFileBrowse")
					.setAttribute("onclick",
							"getAllDirectory('idLeftFileSelect','fasta,fasta_filtered,fa');");
		} else {
			document
					.getElementById("idSingleFileBrowse")
					.setAttribute("onclick",
							"getAllDirectory('idSingleFileSelect','fasta,fasta_filtered,fa');");
		}
	} else {
		if (idInputReadType == "Paired") {
			document
					.getElementById("idRightFileBrowse")
					.setAttribute("onclick",
							"getAllDirectory('idRightFileSelect','fastq,fastq_filtered,fq');");
			document
					.getElementById("idLeftFileBrowse")
					.setAttribute("onclick",
							"getAllDirectory('idLeftFileSelect','fastq,fastq_filtered,fq');");
		} else {
			document
					.getElementById("idSingleFileBrowse")
					.setAttribute("onclick",
							"getAllDirectory('idSingleFileSelect','fastq,fastq_filtered,fq');");
		}
	}
}
...
function onChangeHybridCheckBox(hybridCb) {
	if (hybridCb.checked) {
		if (!document.getElementById("idOptionalSelect2").checked) {
			...
			$("#selectOptionalFormGroup4").fadeOut("slow");
		}
...
function onChangeRefferenceCheckBox(refferenceCb) {
	if (refferenceCb.checked) {
		...
		$("#selectOptionalFormGroup4").fadeIn("show");
