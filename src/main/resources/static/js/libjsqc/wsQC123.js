	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				if (tempOutput != response.message) {
					
					tempOutput = response.message;
					qcOutput = qcOutput + "<br/>" + tempOutput;
...
function submitQC() {
	if (validateInputfolderNameQC() && validateFilesQc()&&validateProjectNameFolder()&&validateOutputFolder()) {
		var xhr = new XMLHttpRequest();
...
function getFormDataQC() {
	...
	formData.append("onlyStat", document.getElementById("idCheckBoxOnlyStats").checked);
	...
	formData.append("outputFolderName", jarray[document.getElementById("idOutputDirSelect").innerHTML]);
...
function showFilesDivPaired(pairedFiles,filesSelectedSingle,indexQc) {
	...
	var closeDiv = document.createElement("div");
	closeDiv.setAttribute("class", "col-sm-12 classSelectedFilesDiv");
	closeDiv.innerHTML = "<span id='close' class='col-sm-1 btn btn-default' onclick='closeDivQC(this,"+indexQc+");'>x</span>";
