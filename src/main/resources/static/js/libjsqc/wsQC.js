...
// }, 5000);
/*
 * function sentRequestToServer() { var xhr = new XMLHttpRequest();
 * xhr.open('GET', URL_LOG_QC, true); xhr.send(); xhr.onreadystatechange =
 * function() { var response = eval("(" + xhr.responseText + ")"); if
 * (xhr.readyState == 4) { if (xhr.status == 200) { // alert("You haveTTTT=>" +
 * xhr.responseText); if (tempOutput != response.message) {
 * 
 * tempOutput = response.message; qcOutput = qcOutput + "<br/>" + tempOutput;
 * $("#454QcSection").fadeOut('slow'); $("#qcCmdQutput").fadeIn('slow'); var
 * objDiv = document.getElementById("454QcCmdOutput"); objDiv.innerHTML =
 * qcOutput; $("#454QcCmd").animate({ scrollTop : $('#454QcCmd')[0].scrollHeight },
 * 1000); } } else { } } }; }
 */
function submitQC() {
