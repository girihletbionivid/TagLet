			...
			if (xhr.readyState == 4) {
				if (xhr.status == 200) {
					...
					if (response.userId != -1) {
						...
						startSession(response.userId, response.userName,
								response.userDir,response.emailId);
...
$(document).ready(
		...
		//	alert("====>"+localStorage.userName);
			...
			/*	document.getElementById("aHome").click();*/
				...
				document.getElementById("userDivOnlyName").innerHTML = ""
					+ localStorage.userName;
				document.getElementById("userDivName").innerHTML = "Welcome "
					+ localStorage.userName;
			document.getElementById("userDivEmail").innerHTML = ""
				...
				startSession(localStorage.userId, localStorage.userName,
						localStorage.userDir,localStorage.userEmail);
			}
			// startSession(localStorage.userId, localStorage.userName);
			// $("#aHome").click();
...
$(document).keypress(function(e) {
	...
	if (e.which == 13) {
		...
		if(document.getElementById("forgetModel")!=null){
